...
const MobileCarousel = ({ projects, projectsVisible }) => {
  ...
  return (
    <div className="mb-16">
        ...
        >
          {projects.map((project, index) => (
            <div key={index} className="w-full flex-shrink-0 px-4">
              <div className="project-card p-4 rounded-lg hover-lift opacity-100 translate-y-0 scale-100">
                ...
                <div className="flex gap-3">
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-600/50 hover:border-gray-500/50 rounded-lg transition-all duration-300 text-gray-300 hover:text-white group text-sm"
                  >
                  ...
                  {project.liveUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-purple-600/20 hover:bg-purple-600/30 border border-purple-400/30 hover:border-purple-400/50 rounded-lg transition-all duration-300 text-purple-300 hover:text-purple-200 group text-sm"
                    >
      ...
      <div className="flex items-center justify-between mt-6">
        <button
          onClick={prevProject}
          className="p-2 rounded-full bg-gray-800/50 hover:bg-gray-700/50 border border-gray-600/50 hover:border-purple-400/50 transition-all duration-300"
        >
...
export default function Projects() {
    ...
    const loadGitHubData = async () => {
      try {
        ...
        if (contributionData.contributions) {
          ...
          const currentStreak = calculateCurrentStreakSimple(contributionData.contributions)
  ...
  // Better current streak calculation that handles "today" more gracefully
  const calculateCurrentStreakSimple = (contributions) => {
    let streak = 0
    const today = new Date()
    ...
    // Start checking from yesterday (day -1) to avoid incomplete "today"
    // This gives a more meaningful streak since today might not be finished
    for (let i = 1; i < 365; i++) {
      ...
      if (count > 0) {
        ...
        // Break on first day with no contributions
        break
      }
    }
    
    console.log('Current streak (excluding today):', streak)
    return streak
  }
  ...
  return (
    <section id="projects" className="py-20 px-6 relative overflow-hidden" ref={sectionRef}>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* Projects - Mobile Carousel or Desktop Grid */}
        {isMobile ? (
          <MobileCarousel projects={dummyProjects} projectsVisible={projectsVisible} />
        ) : (
          ...
          >
            ...
            {loading ? (
              ...
              <div className="max-w-6xl mx-auto">
                {/* Hide calendar on mobile, show on md and up */}
                {!isMobile && (
                  <>
                    {/* Month Labels */}
